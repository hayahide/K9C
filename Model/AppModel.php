...
class AppModel extends Model {
	...
	private $logglyToken="ee7f90dc-a987-4fac-aafa-9685a76964b8";
	...
	private function __escape(&$data)
	{

		$types=$this->getColumnTypes();
		$allow_escape_column_types=array("string","text");
		$noescapes=array("hash");

		foreach($data as $column=>$value){
		
			if(!isset($types[$column]) OR !in_array($types[$column],$allow_escape_column_types)) continue;
			if(in_array($column,$noescapes)) continue;
			if(empty($value)) continue;
			$data[$column]=Sanitize::escape($data[$column]);
		}
	}
	...
	function getLastQuery(){
		...
		$datasource=$this->getDbo();
		$log=$datasource->getLog();
		if(!isset($log["log"])) return;
		$last_query=$log["log"][count($log["log"])-1];
		return $last_query;
	}

	function logglyQueryLog($sql,$tag){
	
		if(!defined("CLIENT")) return;
		$token=$this->logglyToken;
		$log=new Logger(CLIENT);
		$loggly=new LogglyHandler("{$token}/tag/{$tag}",Logger::INFO);
		$log->pushHandler($loggly);
		return $log->addInfo($sql);
	}
...
# ------------------------------------------------------------------------------------------------------
	...
	*/
	...
	#■DBバックアップ
	function mysqlDump($params=array()){

		$backup_dir=$params["backup_dir"];
		$db        =$params["db"];
		$dump_cmd  =(isset($params["dump_cmd"])?$params["dump_cmd"]:system("which mysqldump"));
		...
		$output_arr=array();
		$user  =MYSQL_DEFAULT_LOGIN;
		$passwd=MYSQL_DEFAULT_PASS;
		$host  =MYSQL_DEFAULT_HOST;
		$psfile=MYSQL_DEFAULT_PS_FILE;
		$command="{$dump_cmd} --defaults-extra-file={$psfile} -h {$host} -u {$user} -t {$db} {$table} > {$backup_file_path} & echo $!";
		ob_start();
		exec($command,$output_arr);
		$res=ob_get_contents();
		ob_end_clean();
		$res["output"]=$output_arr;
		$res["backup_file"]=$backup_file_path;
		return $res;
	}

	public function changeConnection($host,$user,$password,$dbName){
	
		$getConnection=$this->getDbConnection($host,$user,$password,$dbName);
